 ...
 * and  Often it is more efficient to create the objects
...
public class GLFactory {
	...
	public Shape newTreangle(Color canBeNull) {
		Shape s = new Shape(canBeNull);
		s.add(new Vec(0, 0, 0.5f));
		s.add(new Vec(0, 0.5f, 0));
		s.add(new Vec(0.5f, 0, 0));
	...
	public MeshComponent newTexturedSquare(String bitmapName, Bitmap bitmap,
		...
		Log.d(LOG_TAG, "   > bitmap.getHeight()=" + bitmap.getHeight());
		Log.d(LOG_TAG, "   > bitmap.getWidth()=" + bitmap.getWidth());
		Log.d(LOG_TAG, "   > height/width factor=" + f);
		Log.d(LOG_TAG, "   > w2=" + w2);
		Log.d(LOG_TAG, "   > h2=" + h2);
	...
	//fatta da me per impostare il colore della scritta
	public MeshComponent newTextObject(String textToDisplay, Vec textPosition,
									   Context context, GLCamera glCamera, boolean colore) {
		...
		if (colore) {
			...
			//v.setTextColor(new Color(Color.redTransparent().red, Color.redTransparent().green, Color.redTransparent().blue, Color.redTransparent().alpha).toIntARGB());
			v.setTextColor(new Color(255, 167, 0, 1).toIntARGB());
		}
		v.setText(textToDisplay);
